...
import custom from './housing-list-type-1.module.scss';

export const HousingListType_1 = async () => {
  const data: HousingListDTO = await getLocalData(
    'src/components/housing-list/components/housing-list-type-1/data.json',
  );
      ...
      <div className={clsx(common.root, custom.root)}>
        ...
        <ul className={clsx(common.list, custom.list)}>
          {data.content.list.map((item) => (
            ...
            >
              <div className={clsx(common.list_item, custom.list_item_gallery)}>
                <SliderHousingList>
                  {item.image.map((img) => (
                    <Image
                      key={img.url}
